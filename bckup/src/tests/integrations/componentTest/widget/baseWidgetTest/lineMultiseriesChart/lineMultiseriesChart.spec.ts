...
test.describe('lineMultiseries chart complex test', () => {
	...
	test('base render', async({page, context}) => {
		await baseRenderTest(page, context, 'lineMultiseriesChart', mock.baseRender.story, mock.baseRender.data);
	});
	test('Bucket', async({page, context}) => {
		await baseRenderTest(page, context, 'lineMultiseriesChart', mock.Bucket.story, mock.Bucket.data);
	});
	test('FormattingEnums', async({page, context}) => {
		await baseRenderTest(page, context, 'lineMultiseriesChart', mock.FormattingEnums.story, mock.FormattingEnums.data);
	});
});
