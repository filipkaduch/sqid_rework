...
test.describe('sparkline chart complex test', () => {
	...
	test('base render', async({page, context}) => {
		await baseRenderTest(page, context, 'sparklineChart', mock.baseRender.story, mock.baseRender.data);
	});
	test('Bucket', async({page, context}) => {
		await baseRenderTest(page, context, 'sparklineChart', mock.Bucket.story, mock.Bucket.data);
	});
	test('FormattingEnums', async({page, context}) => {
		await baseRenderTest(page, context, 'sparklineChart', mock.FormattingEnums.story, mock.FormattingEnums.data);
	});
});
