...
test.describe('comparison chart complex test', () => {
	...
	test('base render', async({page, context}) => {
		await baseRenderTest(page, context, 'comparisonChart', mock.baseRender.story, mock.baseRender.data);
	});
	test('Bucket', async({page, context}) => {
		await baseRenderTest(page, context, 'comparisonChart', mock.Bucket.story, mock.Bucket.data);
	});
	test('FormattingEnums', async({page, context}) => {
		await baseRenderTest(page, context, 'comparisonChart', mock.FormattingEnums.story, mock.FormattingEnums.data);
	});
});
