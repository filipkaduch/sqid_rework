...
test.describe('bubble chart complex test', () => {
	...
	test('base render', async({page, context}) => {
		await baseRenderTest(page, context, 'bubbleChart', mock.baseRender.story, mock.baseRender.data);
	});
	test('Bucket', async({page, context}) => {
		await baseRenderTest(page, context, 'bubbleChart', mock.Bucket.story, mock.Bucket.data);
	});
	test('FormattingEnums', async({page, context}) => {
		await baseRenderTest(page, context, 'bubbleChart', mock.FormattingEnums.story, mock.FormattingEnums.data);
	});
});
