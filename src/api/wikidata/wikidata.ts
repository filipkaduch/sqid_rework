...
export async function getEntityData(entityId: EntityId, lang?: string, fallback = true, queryEntityId: EntityId = '') {
  const entities = await getEntities([entityId],
                                     ['aliases', 'labels', 'descriptions', 'info',
                                      'claims', 'datatype', 'sitelinks'],
                                     lang,
                                     fallback, queryEntityId)
...
export async function searchEntities(search: string,
  ...
  const langCode = options.lang || i18n.global.locale
  const params = {
