...
import { ClaimsMap } from '@/modules/entities/store/claims/types'
...
export async function getStatistics(lastRefresh: number): Promise<SqidStatistics> {
  ...
  response.dumpDate = `${dumpYear}-${dumpMonth}-${dumpDay}`;

  return response;
}

export async function getRuleSchema(lastRefresh: number): Promise<SqidRuleSchema> {
  const response = await axiosGet(getDataFileURI('rules.schema', lastRefresh))
...
export async function getRelatedPropertiesChunk(chunkId: number, lastRefresh: number) {
  let response
  try {
    response = await axiosGet(getDataFileURI(`properties/related-${chunkId}`, lastRefresh))
  } catch {
    return {}
  }
...
export async function getRelatedProperties(lastRefresh: number): Promise<RelatednessMapping> {
  const response = await axiosGet(getDataFileURI('properties/related', lastRefresh))
...
export async function getPropertyClassification(lastRefresh: number): Promise<Map<EntityId, PropertyClassification>> {
  const response = await axiosGet(getDataFileURI('properties/classification', lastRefresh));
  const classification = new Map<EntityId, PropertyClassification>();

  for (const [entityId, kind] of Object.entries(response)) {
...
export type PropertyClassifier = (entityId: EntityId, queryEntityId: EntityId) => PropertyClassification
...
export async function getClassHierarchyChunk(chunkId: number, lastRefresh: number) {
  ...
  let response
  try {
    response = await axiosGet(getDataFileURI(`classes/hierarchy-${chunkId}`, lastRefresh))
  } catch {
    return chunk
  }

  // @ts-ignore
  for (const [entityId, data] of Object.entries(response as { [key: string]: SqidHierarchyRecord })) {
...
export async function getPropertyUsage(lastRefresh: number) {
  ...
  const response = await axiosGet(getDataFileURI('properties/usage', lastRefresh))

  // @ts-ignore
  for (const [propertyId, record] of Object.entries(response as ResultList<SqidPropertyUsageRecord>)) {
