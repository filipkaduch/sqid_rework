<template>
	<app-tooltip class="inherit-font-color" :show="tooltip !== ''">
		<component
			:is="componentTag"
			tabindex="0"
			:contenteditable="isEditable"
			class="m-0 ds-text"
			:class="[`app-text-${color}`]"
		...
		<template #tooltip>
			<app-box :class="{'wrap-tooltip': wrapTooltip}">
				{{ tooltip }}
			</app-box>
		</template>
	</app-tooltip>
...
import {fontStyles, spacingStyles} from '@/styles/values/exportStyles';
// eslint-disable-next-line no-unused-vars
...
export default defineComponent({
	...
	props: {
		...
		color: {
			type: String as PropType<AppColorsTypes | null>,
	...
	setup(props) {
		const componentTag = computed(() => {
			switch (props.variant) {
				...
				case 'headline1light':
				case 'app-headline1':
				case 'app-s-headline1':
					return 'h1';
				case 'headline2':
				case 'app-headline2':
				case 'app-s-headline2':
					return 'h2';
				case 'subheadline':
				case 'subheadlineRegular':
				case 'app-headline3':
				case 'app-s-headline3':
					return 'h3';
				case 'body':
				case 'bodyMedium':
					return 'span';
